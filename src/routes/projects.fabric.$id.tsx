...
export const Route = createFileRoute("/projects/fabric/$id")({
  loader: ({ params }) => {
    const m = params.id.match(/^fab-(\d{2})$/);
    if (!m) throw notFound();
    const n = parseInt(m[1], 10);
    if (n < 1 || n > 11) throw notFound();
    return { number: n, code: `FAB-${m[1]}` };
  },
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [
          { title: "Fabric Project — Not Found" },
          { name: "robots", content: "noindex" },
        ],
      };
    }
    const title = `Fabric Project ${String(loaderData.number).padStart(2, "0")} — Jassim Mohamed Kutty`;
    return {
      meta: [
        { title },
        { name: "description", content: "Microsoft Fabric case study. Detailed write-up coming soon." },
      ],
    };
  },
  component: FabricProjectDetail,
  notFoundComponent: ProjectNotFound,
});

function FabricProjectDetail() {
        ...
        <div className="mx-auto max-w-4xl px-4">
          ...
          <div className="glass rounded-2xl p-8 md:p-10">
            ...
            <h1 className="text-3xl md:text-5xl font-bold mt-4 mb-3">
              Fabric Project <span className="gradient-text">{nn}</span>
            </h1>
